...
namespace oojjrs.oh
{
    public class EntityModelBindingT<IdType, EntityType, ModelType> where ModelType : MonoBehaviour
    {
        ...
        public void Bind(IdType id, EntityType entity, ModelType model)
        {
            if (model == null)
            {
                Debug.LogWarning($"{nameof(EntityModelBindingT<IdType, EntityType, ModelType>)}> Bind ����: ���� ��� �ִ�");
                return;
            }
        ...
        private void WarnIfBindingExists(IdType id, EntityType entity, ModelType model)
        {
            if (_idEntryTable.ContainsKey(id))
                Debug.LogWarning($"{nameof(EntityModelBindingT<IdType, EntityType, ModelType>)}> Bind ���: ���� ID�� �̹� �ִ�");

            if (_entityEntryTable.ContainsKey(entity))
                Debug.LogWarning($"{nameof(EntityModelBindingT<IdType, EntityType, ModelType>)}> Bind ���: ���� Entity�� �̹� �ִ�");

            if (_modelEntryTable.ContainsKey(model))
                Debug.LogWarning($"{nameof(EntityModelBindingT<IdType, EntityType, ModelType>)}> Bind ���: ���� Model�� �̹� �ִ�");
        }
    }
}
